    ...
    return (
      <section className="bg-[#0a0a0a] py-24 sm:py-32">
        <div className="mx-auto max-w-lg px-4 text-center">
          ...
          <h1 className="mt-4 font-heading text-2xl font-bold text-white">Application received</h1>
          ...
          <form onSubmit={handleSubmit(onSubmit as any)} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                type="text"
                {...register("full_name")}
                className="mt-1 block w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none placeholder:text-white/30 focus:border-gold focus:ring-1 focus:ring-gold"
              />
              {errors.full_name && (
            ...
            <div>
              ...
              {errors.date_of_birth && (
                <p className="mt-1 text-xs text-red-400">{errors.date_of_birth.message}</p>
              )}
            </div>

            {/* Years Experience */}
            <div>
              <label htmlFor="years_experience" className="block text-sm font-medium text-white">
              ...
              {errors.years_experience && (
                <p className="mt-1 text-xs text-red-400">{errors.years_experience.message}</p>
              )}
            </div>

            {/* Prior Teams */}
            <div>
              <label htmlFor="prior_teams" className="block text-sm font-medium text-white">
            ...
            {/* Position Preference */}
            <div>
              <label htmlFor="position_preference" className="block text-sm font-medium text-white">
                Position preference
              </label>
              <select
                id="position_preference"
                {...register("position_preference")}
                className="mt-1 block w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none focus:border-gold focus:ring-1 focus:ring-gold"
              >
                <option value="" className="bg-[#141414]">Select position</option>
                {POSITIONS.map((pos) => (
                  <option key={pos} value={pos} className="bg-[#141414]">
                    {pos}
                  </option>
                ))}
              </select>
              {errors.position_preference && (
                <p className="mt-1 text-xs text-red-400">{errors.position_preference.message}</p>
              )}
            </div>
